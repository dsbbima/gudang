...
    
    // 🔥 Panggil fungsi pemuatan data dari JSON lokal
...
// =================================================================
// FUNGSI PENGAMBILAN DATA (MODIFIKASI UTAMA)
// =================================================================
async function loadInventoryDataFromJSON() {
    ...
    if (cachedData && cachedTimestamp) {
        ...
        if (cacheAge < CACHE_DURATION_MS) {
            // Cache belum kedaluwarsa (misalnya 24 jam)
            try {
                inventoryDataCache = transformDataToUppercase(JSON.parse(cachedData));
                initializeInventoryState(true); 
                displayMessage(`✅ Data produk dari **Local Storage** berhasil dimuat. (Cache Lokal Segar)`, 'success');
                return; // Langsung kembali jika cache segar
            } catch (e) {
    ...
    // 2. Ambil data dari inventory.json lokal
    ...
    try {
        ...
        const serverLatestTimestamp = new Date().getTime(); // Gunakan waktu saat ini sebagai timestamp baru
        ...
        displayMessage(`✅ Data produk baru berhasil dimuat dari **inventory.json** (${new Date(serverLatestTimestamp).toLocaleTimeString()}). Halo, **${activeUser}**!`, 'success');
        ...
        if (Object.keys(inventoryDataCache).length === 0) {
            ...
            displayMessage(`⚠️ Gagal memuat data terbaru dari JSON: ${error.message}. Menggunakan data cache lama.`, 'error');
        }
    }
}
...
    }
    
    // Logika dummy/minimalis untuk menginisialisasi scanner
    window.Quagga = window.Quagga || {
        init: (config, callback) => { callback(); },
        start: () => { qrScannerContainer.innerHTML = '<div class="scanner-placeholder">🎥 QR/Barcode Scanner Aktif (Integrasi QuaggaJS)</div>'; },
        stop: () => { qrScannerContainer.innerHTML = ''; },
        onDetected: (callback) => { console.log("Quagga.onDetected terpasang (dummy)"); }
    };
    
...
            },
            decoder: { readers: ["code_128_reader", "ean_reader", "upc_reader", "code_39_reader", "qr_code_reader"] },
            locate: true,
        }, (err) => {
            if (err) {
